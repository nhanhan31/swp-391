...
const CustomerPage = () => {
  ...
  const checkEmailVerificationStatus = async (email) => {
    try {
      // Gọi API GetAll vừa thêm
      const response = await emailVerificationAPI.getAll();

      // Kiểm tra xem API trả về có phải mảng không và tìm email tương ứng
      if (Array.isArray(response)) {
        // Tìm bản ghi có email trùng khớp VÀ isVerified = true
        const matchedRecord = response.find(
          r => r.email?.toLowerCase() === email?.toLowerCase() && r.isVerified === true
        );
        return !!matchedRecord; // Trả về true nếu tìm thấy, ngược lại false
      }
      return false;
    } catch (error) {
      console.error("Lỗi khi kiểm tra trạng thái xác thực:", error);
      return false; // Nếu lỗi API, mặc định coi như chưa xác thực để an toàn
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Lưu tạm dữ liệu để dùng sau khi verify xong
      setPendingFormData(customerPayload);

      // Nếu là Tạo mới hoặc Sửa Email -> Cần kiểm tra xác thực
      if (isNew || emailChanged) {
        setVerifyLoading(true); // Hiện loading

        // 1. GỌI HÀM CHECK
        const isVerified = await checkEmailVerificationStatus(values.email);

        setVerifyLoading(false); // Tắt loading

        if (isVerified) {
          // 2. NẾU ĐÃ VERIFY: Bỏ qua OTP, Lưu luôn
          console.log("Email đã được xác thực trước đó. Bỏ qua OTP.");
          await processPendingSubmit(customerPayload);
        } else {
          // 3. NẾU CHƯA VERIFY: Hiện Modal OTP
          setEmailToVerify(values.email);
          setIsVerifyModalOpen(true);
        }
      } else {
        // Trường hợp sửa thông tin khác (không sửa email) -> Lưu luôn
        await processPendingSubmit(customerPayload);
      }
